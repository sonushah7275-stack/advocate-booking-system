...
const Login = () => {
  ...
  const { setDToken } = useContext(AdvocateContext);
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (state === "Admin") {
        ...
        const { data } = await axios.post(backendUrl + "/api/advocate/login", {
          email,
          password,
        });
        if (data.success) {
    ...
    <form onSubmit={onSubmitHandler} className="min-h-[80vh] flex items-center">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-sm shadow-lg">
        <p className="text-2xl font-semibold m-auto">
          <span className="text-amber-400">{state} </span>Login
        </p>
